...
def get_stock_fundamentals(sym):
    ...
    if not report_url:
        ...
        if not morning_overview_url or str(requests.get("http:" + morning_overview_url[0])) != "<Response [200]>":
            return 0, 0, 0, 0, 0, 0
        else:

            '#needs http because without it, it gives a error: MissingSchema: Invalid URL : No schema supplied'
            morning_overview_url[0] = "http:" + morning_overview_url[0]
            '''takes first element of the list'''
